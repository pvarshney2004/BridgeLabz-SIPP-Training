...
public class SmartCityApp {
    ...
    static class BusService implements TransportService {
        ...
        BusService(String id, String route, LocalTime departure, double fare, boolean active) {
            this.id = id; this.route = route; this.departure = departure; this.fare = fare; this.active = active;
        }
        public String getId() { return id; }
        public String getType() { return "Bus"; }
        public String getRoute() { return route; }
        public LocalTime getDeparture() { return departure; }
        public double getBaseFare() { return fare; }
    ...
    static class Passenger {
        ...
        Passenger(String id, String name, String route, LocalTime travelTime, double distanceKm, double paidFare) {
            this.id = id; this.name = name; this.route = route; this.travelTime = travelTime; this.distanceKm = distanceKm; this.paidFare = paidFare;
        }
        public String getRoute() { return route; }
    ...
    public static void main(String[] args) {
        ...
        List<Passenger> passengers = Arrays.asList(
            new Passenger("P1", "Raman", "R1", LocalTime.of(9, 0), 5.0, fareCalc.calculateFare(services.get(0), 5.0, LocalTime.of(9,0))),
            new Passenger("P2", "Sonia", "R1", LocalTime.of(8,50), 3.0, fareCalc.calculateFare(services.get(2), 3.0, LocalTime.of(8,50))),
            new Passenger("P3", "Ajay", "R2", LocalTime.of(9,16), 4.0, fareCalc.calculateFare(services.get(1), 4.0, LocalTime.of(9,16))),
            new Passenger("P4", "Deepa", "R3", LocalTime.of(9,5), 12.0, fareCalc.calculateFare(services.get(3), 12.0, LocalTime.of(9,5))),
            new Passenger("P5", "Vikram", "R1", LocalTime.of(18,0), 8.0, fareCalc.calculateFare(services.get(0), 8.0, LocalTime.of(18,0)))
        );

        // ---------- A. Filter & Sort services using lambda (e.g., earliest departure then lowest fare) ----------
        ...
        services.forEach(s -> System.out.println("Dashboard: " + s.getType() + " " + s.getId() + " -> " + s.getDeparture()));
        ...
        // ---------- D. Partitioning: peak vs non-peak trips (Collectors.partitioningBy) ----------
        ...
        Map<String, DoubleSummaryStatistics> fareStats = passengers.stream()
                .collect(Collectors.groupingBy(Passenger::getRoute, Collectors.summarizingDouble(Passenger::getPaidFare)));
        ...
        // ---------- J. Demonstrate easy expansion: add FerryService class (lambda style) ----------
        ...
        TransportService ferry = new TransportService() {
            public String getId() { return "F1"; }
            public String getType() { return "Ferry"; }
            public String getRoute() { return "R5"; }
            public LocalTime getDeparture() { return LocalTime.of(10, 0); }
            public double getBaseFare() { return 120.0; }
